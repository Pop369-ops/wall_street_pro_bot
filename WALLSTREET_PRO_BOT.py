...
• Entry: [المستوى]
• Stop Loss: [المستوى] ← (-X%)
• Take Profit 1: [المستوى] ← (+X%)
• Take Profit 2: [المستوى] ← (+X%)
• R:R Ratio: [1:X]
• حجم المركز المقترح: [X% من رأس المال]
...
def build_recommendation(asset: str = "Gold", chat_id: str = None) -> Dict:
    """يبني توصية كاملة من 3 AIs + إجماع + يحفظها في DB."""
...
🛡️ **إدارة المخاطر النهائية (Entry/SL/TP1/TP2):**
        ...
        db_exec(
            """INSERT INTO recommendations 
               (chat_id, asset, action, entry_price, stop_loss, 
                take_profit_1, take_profit_2, confidence, timeframe, 
                reasoning, created_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (chat_id, asset, rec_data["action"],
             rec_data.get("entry"), rec_data.get("sl"),
             rec_data.get("tp1"), rec_data.get("tp2"),
...
async def cmd_start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    msg = (
        "🌟 *WALL STREET PRO BOT V4*\n"
        "_3 AI Brains + Smart Money + Options + Macro_\n\n"
        "━━━━━━━━━━━━━━━━━━━\n"
        "🎯 *التوصيات الذكية:*\n"
        "`توصية` — توصية الذهب الكاملة\n"
...
def main():
    ...
    print("=" * 70)
    print("  WALL STREET PRO BOT V4 — UNIFIED — Running ✅")
    print("=" * 70)
